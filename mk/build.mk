...
define gocross
	...
	GOOS=$(1) GOARCH=$(2) CGO_ENABLED=0 go build \
		-o $(PREFIX)/bin/terraform-provider-sacloud-${os.$(1)}-${arch.$(2)}$(call extension,$(GOOS)) \
...
# Build just driver
build-driver: $(PREFIX)/bin/terraform-provider-sacloud

build-x: $(shell find . -type f -name '*.go')
	$(foreach GOARCH,$(TARGET_ARCH),$(foreach GOOS,$(TARGET_OS),$(call gocross,$(GOOS),$(GOARCH),./main.go)))
